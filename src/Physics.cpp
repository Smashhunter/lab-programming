...
Physics::Physics(double timePerTick)
  : timePerTick{ timePerTick }
{
}

void
Physics::setWorldBox(const Point& topLeft, const Point& bottomRight)
{
...
void
Physics::update(std::vector<Ball>& balls, std::vector<Dust>& dusts, const size_t ticks) const
{
...
void
Physics::collideBalls(std::vector<Ball>& balls, std::vector<Dust>& dusts) const
{
    ...
    if(a->isCollidable()) {
      for (auto b = std::next(a); b != balls.end(); ++b) {
        ...
        if (distanceBetweenCenters2 < collisionDistance2) {
          ...
          dusts.push_back(Dust(Velocity(Point(30, 30)), collisionPoint, Color(0.2, 0.3, 0.4)));
          dusts.push_back(Dust(Velocity(Point(30, -30)), collisionPoint, Color(0.2, 0.3, 0.4)));
          dusts.push_back(Dust(Velocity(Point(-30, 30)), collisionPoint, Color(0.2, 0.3, 0.4)));
          dusts.push_back(Dust(Velocity(Point(-30, -30)), collisionPoint, Color(0.2, 0.3, 0.4)));
        }
...
void
Physics::collideWithBox(std::vector<Ball>& balls) const
{
  for (Ball& ball : balls) {
...
void
Physics::move(std::vector<Ball>& balls) const
{
  for (Ball& ball : balls) {
...
Point
Physics::processCollision(Ball& a,
                          Ball& b,
